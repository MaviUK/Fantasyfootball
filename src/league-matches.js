...
function pitchPlayer(p,pos,index,count){const r=Math.round(Number(p.overall_rating||0));return `<div class="pitch-player pos-${pos.toLowerCase()} pos-${index+1}-of-${count}"><span class="pitch-rating ${ratingClass(r)}">${r}</span><span class="pitch-avatar">${initials(p.player_name)}</span><strong>${shortName(p.player_name)}</strong><small>${p.game_position||''}</small></div>`}
function pitchHalf(players,side){const starters=(players||[]).filter(p=>p.slot_type==='starter');const groups={GK:[],CD:[],MD:[],ATT:[]};starters.forEach(p=>(groups[p.game_position]||groups.MD).push(p));return `<div class="formation-grid ${side}-formation">${['GK','CD','MD','ATT'].map(pos=>groups[pos].map((p,i)=>pitchPlayer(p,pos,i,groups[pos].length)).join('')).join('')}</div>`}
