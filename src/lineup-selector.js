function enhanceLineup(){
 ...
 page.appendChild(sheet); sheet.querySelector('.sheet-content').appendChild(available);
 ...
 const findStarterRow=(pos,playerName)=>{const wanted=normalise(playerName);return [...(xiCol?.querySelectorAll('.lineup-player')||[])].find(row=>{const rowPos=row.querySelector('.avatar')?.textContent?.trim(),current=rowName(row);return rowPos===pos&&(current===wanted||current.endsWith(wanted)||wanted.endsWith(current))})};
 const findAvailableRow=(pos,playerName)=>{const wanted=normalise(playerName);return rows().find(row=>{const rowPos=row.querySelector('.avatar')?.textContent?.trim(),current=rowName(row);return rowPos===pos&&(current===wanted||current.endsWith(wanted)||wanted.endsWith(current))})};
 ...
 const close=()=>{sheet.classList.remove('open');document.body.classList.remove('picker-open');sheet.dataset.position='';sheet.dataset.playerName='';sheet.dataset.swapBypass='';filter('')};
 ...
 pitch.addEventListener('click',e=>{const card=e.target.closest('.pitch-player.filled');if(!card)return;open(card.querySelector('span')?.textContent?.trim()||'',card.querySelector('strong')?.textContent?.trim()||'')});
 sheet.addEventListener('click',e=>{
   const start=e.target.closest('.lineup-actions button:first-child'); if(!start)return;
   if(sheet.dataset.swapBypass==='1'){sheet.dataset.swapBypass='';setTimeout(close,120);return}
   const pos=sheet.dataset.position||'',outgoingName=sheet.dataset.playerName||'';
   if(!pos||!outgoingName){setTimeout(close,120);return}
   const chosenName=start.closest('.lineup-player')?.querySelector('strong')?.textContent?.trim()||'';
   const removeButton=findStarterRow(pos,outgoingName)?.querySelector('.lineup-remove'); if(!removeButton||!chosenName)return;
   e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();
   removeButton.click();
   let tries=0;
   const finish=()=>{tries++;const freshStart=findAvailableRow(pos,chosenName)?.querySelector('.lineup-actions button:first-child');if(freshStart){sheet.dataset.swapBypass='1';freshStart.click();return}if(tries<20)setTimeout(finish,50);else close()};
...
new MutationObserver(enhanceLineup).observe(document.documentElement,{childList:true,subtree:true});enhanceLineup();
